...
public:
    LoadingOverlay(QWidget *parent) : QWidget(parent) {
        ...
        label->setStyleSheet(QString("color: %1; font-family: 'Consolas'; font-size: 20px; font-weight: bold;").arg(hex));
...
void ProfilePanel::show_slot_options(int slot_id, Button* btn) {
    ...
    hb->addWidget(load); hb->addWidget(save); hb->addWidget(del); hb->addWidget(back);
    c->addLayout(hb);
    l->addWidget(f);
...
void ProfilePanel::open_keyboard(int slot_id, Button* btn) {
    ...
    TacticalKeyboard *kb = new TacticalKeyboard((current_name == "EMPTY" ? "" : current_name), &kb_dlg);
...
void ProfilePanel::refresh_theme() {
    ...
    status_label->setStyleSheet(QString("color: %1; font-family: 'Consolas'; font-size: 16px; font-weight: bold;").arg(h));
