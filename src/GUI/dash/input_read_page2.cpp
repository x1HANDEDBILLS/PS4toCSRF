// LOCATION: /home/user/ACE/src/GUI/dash/input_read_page2.cpp
...
void setup_input_read_page2_content(QWidget* p) {
    ...
    QObject::connect(t, &QTimer::timeout, [=]() {
        ...
        for(int s=0; s<16; s++) {
            ...
            if(check.connected && QString::fromStdString(check.path).endsWith(".2")) {
                dev = check;
                found = true;
                break;
            }
        ...
        if(found) {
            status->setText(QString::fromStdString(dev.name).toUpper() + " [PORT 2]");
            ...
            status->setText("PORT 2: DISCONNECTED");
