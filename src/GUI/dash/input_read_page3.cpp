// LOCATION: /home/user/ACE/src/GUI/dash/input_read_page3.cpp
...
void setup_input_read_page3_content(QWidget* p) {
    ...
    QObject::connect(t, &QTimer::timeout, [=]() {
        ...
        for(int s=0; s<16; s++) {
            ...
            if(check.connected && QString::fromStdString(check.path).endsWith(".3")) {
                dev = check;
                found = true;
                break;
            }
        ...
        if(found) {
            status->setText(QString::fromStdString(dev.name).toUpper() + " [PORT 3]");
            ...
            status->setText("PORT 3: DISCONNECTED");
