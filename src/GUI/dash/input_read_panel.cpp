...
InputReadPanel::InputReadPanel(QWidget *parent) : QFrame(parent) {
    ...
    for(int i = 0; i < 4; ++i) {
        ...
        else if(i==1) setup_input_read_page2_content(p);
        else if(i==2) setup_input_read_page3_content(p);
        else if(i==3) setup_input_read_page4_content(p);
        pages->addWidget(p);
    }
    ...
    page_label = new QLabel("1 / 4"); page_label->setAlignment(Qt::AlignCenter); page_label->setFixedWidth(100);
...
void InputReadPanel::next_page() { pages->setCurrentIndex((pages->currentIndex() + 1) % 4); update_page_ui(); }
void InputReadPanel::prev_page() { int idx = pages->currentIndex() - 1; if (idx < 0) idx = 3; pages->setCurrentIndex(idx); update_page_ui(); }
...
void InputReadPanel::refresh_theme() {
    ...
    header_title->setStyleSheet(QString("color: %1; font-size: 32px; font-weight: bold;").arg(silver));
