...
DashboardPanel::DashboardPanel(QWidget *parent) : QWidget(parent) {
    ...
    QList<QWidget*> panels = {input_gui, settings_gui, rc_type_gui, profile_gui, rc_stack, input_stack, rc_map_stack};
    for(auto p : panels) { p->setFixedSize(834, 600); p->hide(); }
    ...
    connect(anim_side, &QPropertyAnimation::finished, this, [this](){
        if(!sidebar_open) { sidebar->hide(); shield->hide(); settings_btn->raise(); }
    });
    refresh_theme();
}
...
void DashboardPanel::refresh_theme() {
    ...
    sidebar->setStyleSheet(QString("QFrame { background-color: rgba(5, 5, 5, 220); border-left: 2px solid %1; }").arg(accent));
}
...
void DashboardPanel::update_panel_positions() {
    ...
    int ty = (h - 600) / 2;
    QList<QWidget*> ps = {input_gui, settings_gui, rc_type_gui, profile_gui, rc_stack, input_stack, rc_map_stack};
    for(auto p : ps) p->move(tx, ty);
}

void DashboardPanel::hide_all_guis() {
    input_gui->hide(); settings_gui->hide(); rc_type_gui->hide();
    profile_gui->hide(); rc_stack->hide(); input_stack->hide(); rc_map_stack->hide();
    input_open = settings_open = rc_type_open = profile_open = rc_tune_open = input_tune_open = rc_map_open = false;
}

void DashboardPanel::smart_back() {
    if (rc_map_open) { rc_map_open = false; rc_map_stack->hide(); }
    else if (rc_tune_open) { rc_tune_open = false; rc_stack->hide(); }
    else if (input_tune_open) { input_tune_open = false; input_stack->hide(); }
    else if (input_open) { input_open = false; input_gui->hide(); }
    else if (rc_type_open) { rc_type_open = false; rc_type_gui->hide(); }
    else if (profile_open) { profile_open = false; profile_gui->hide(); }
    else if (settings_open) { settings_open = false; settings_gui->hide(); }
    else if (sidebar_open) {
        sidebar_open = false;
        anim_side->stop(); anim_side->setEndValue(QPoint(width(), 0));
        anim_side->start();
    }
...
void DashboardPanel::toggle_sidebar() {
    ...
    if (!sidebar_open) {
        sidebar_open = true;
        shield->setGeometry(0, 0, w, h); shield->show(); sidebar->show(); 
        shield->raise(); settings_btn->raise(); sidebar->raise();
        anim_side->stop(); anim_side->setStartValue(QPoint(w, 0));
...
void DashboardPanel::toggle_input_gui() { if(!input_open) { hide_all_guis(); input_open = true; input_gui->show(); } else { input_open = false; input_gui->hide(); } update_panel_positions(); }
void DashboardPanel::toggle_settings_gui() { if(!settings_open) { hide_all_guis(); settings_open = true; settings_gui->show(); } else { settings_open = false; settings_gui->hide(); } update_panel_positions(); }
void DashboardPanel::toggle_rc_type_gui() { if(!rc_type_open) { hide_all_guis(); rc_type_open = true; rc_type_gui->show(); } else { rc_type_open = false; rc_type_gui->hide(); } update_panel_positions(); }
void DashboardPanel::toggle_profile_gui() { if(!profile_open) { hide_all_guis(); profile_open = true; profile_gui->show(); } else { profile_open = false; profile_gui->hide(); } update_panel_positions(); }
void DashboardPanel::toggle_rc_tune() { if(!rc_tune_open) { hide_all_guis(); rc_tune_open = true; rc_stack->show(); } else { rc_tune_open = false; rc_stack->hide(); } update_panel_positions(); }
void DashboardPanel::toggle_input_tune() { if(!input_tune_open) { hide_all_guis(); input_tune_open = true; input_stack->show(); } else { input_tune_open = false; input_stack->hide(); } update_panel_positions(); }
void DashboardPanel::toggle_rc_map() { if(!rc_map_open) { hide_all_guis(); rc_map_open = true; rc_map_stack->show(); } else { rc_map_open = false; rc_map_stack->hide(); } update_panel_positions(); }
