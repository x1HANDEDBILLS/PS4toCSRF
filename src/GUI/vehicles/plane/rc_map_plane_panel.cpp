// LOCATION: /home/user/ACE/src/GUI/vehicles/plane/rc_map_plane_panel.cpp
#include "vehicles/plane/rc_map_plane_panel.h"
#include "vehicles/plane/rc_map_plane_page1.h" // Our new Page 1 header
...
RCMapPlanePanel::RCMapPlanePanel(QWidget *parent) : QFrame(parent) {
    ...
    header_title = new QLabel("PLANE SOURCE MAPPING");
    ...
    page_titles = {"SOURCES", "MIXING", "TRIMS", "CURVES", "FAILSAFE"};
    
    for(int i = 0; i < 5; ++i) {
        QWidget *p = new QWidget();
        if(i == 0) {
            setup_rc_map_plane_page1_content(p); // <--- INJECTING OUR PAGE 1 HERE
        } else {
    ...
    prev_btn = new Button("<", this, true); prev_btn->setFixedSize(80, 55);
    page_label = new QLabel("1 / 5"); page_label->setAlignment(Qt::AlignCenter); page_label->setFixedWidth(100);
    next_btn = new Button(">", this, true); next_btn->setFixedSize(80, 55);
    
    footer_layout->addStretch(); footer_layout->addWidget(prev_btn);
    footer_layout->addWidget(page_label); footer_layout->addWidget(next_btn);
...
void RCMapPlanePanel::next_page() { pages->setCurrentIndex((pages->currentIndex() + 1) % 5); update_page_ui(); }
void RCMapPlanePanel::prev_page() { int idx = pages->currentIndex() - 1; if (idx < 0) idx = 4; pages->setCurrentIndex(idx); update_page_ui(); }
