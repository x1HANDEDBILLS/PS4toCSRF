...
public:
    AxisRow(QString name, int axis_index, QWidget* parent) : QWidget(parent), idx(axis_index) {
        ...
        val_box->setStyleSheet(QString("background:#000; color:%1; border:1px solid %1; font-weight:bold; border-radius:4px;").arg(h));
...
void setup_input_tune_plane_page1_content(QWidget* p) {
    ...
    QString headStyle = "color: white; font-weight: bold; font-size: 16px; border-bottom: 2px solid #333; padding-bottom: 5px;";
    ...
    ctrl->expo_slider->setStyleSheet(QString(
        "QSlider::groove:horizontal { height: 16px; background: #222; border-radius: 8px; }"
        "QSlider::handle:horizontal { background: #ffffff; border: 2px solid #888; width: 60px; height: 60px; margin: -22px 0; border-radius: 30px; }"
    ));
    ...
    QString cbStyle = QString(
        ...
        "QCheckBox::indicator { width: 38px; height: 38px; border: 2px solid #444; background: #111; border-radius: 6px; }"
    ...
    for(int i=0; i<6; i++) {
        ...
        QObject::connect(cb, &QCheckBox::toggled, [=](bool checked){ 
            AppState::instance().expo_enabled[i] = checked; 
        });
