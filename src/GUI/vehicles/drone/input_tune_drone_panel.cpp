...
InputTuneDronePanel::InputTuneDronePanel(QWidget *parent) : QFrame(parent) {
    ...
    for(int i = 0; i < 5; ++i) {
        ...
        else if(i==1) setup_input_tune_drone_page2_content(p);
        else if(i==2) setup_input_tune_drone_page3_content(p);
        else if(i==3) setup_input_tune_drone_page4_content(p);
        else if(i==4) setup_input_tune_drone_page5_content(p);
        pages->addWidget(p);
    }
...
void InputTuneDronePanel::next_page() { pages->setCurrentIndex((pages->currentIndex() + 1) % 5); update_page_ui(); }
void InputTuneDronePanel::prev_page() { int idx = pages->currentIndex() - 1; if (idx < 0) idx = 4; pages->setCurrentIndex(idx); update_page_ui(); }
