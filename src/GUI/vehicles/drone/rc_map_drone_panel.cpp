...
RCMapDronePanel::RCMapDronePanel(QWidget *parent) : QFrame(parent) {
    ...
    footer_layout->addWidget(page_label); footer_layout->addWidget(next_btn); footer_layout->addStretch();
...
void RCMapDronePanel::next_page() { pages->setCurrentIndex((pages->currentIndex() + 1) % 5); update_page_ui(); }
void RCMapDronePanel::prev_page() { int idx = pages->currentIndex() - 1; if (idx < 0) idx = 4; pages->setCurrentIndex(idx); update_page_ui(); }
