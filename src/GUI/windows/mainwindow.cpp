...
void MainWindow::regenerate_dithered_bg() {
    ...
    for (int y = 0; y < h; ++y) {
        ...
        for (int x = 0; x < w; ++x) {
            ...
            int r = std::clamp(int(cfg.bg_light.red()   + (cfg.bg_dark.red()   - cfg.bg_light.red())   * ratio + noise), 0, 255);
            int g = std::clamp(int(cfg.bg_light.green() + (cfg.bg_dark.green() - cfg.bg_light.green()) * ratio + noise), 0, 255);
            int b = std::clamp(int(cfg.bg_light.blue()  + (cfg.bg_dark.blue()  - cfg.bg_light.blue())  * ratio + noise), 0, 255);
