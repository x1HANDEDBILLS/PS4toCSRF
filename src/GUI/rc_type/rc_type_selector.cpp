...
RCTypeSelector::RCTypeSelector(QWidget *parent) : QWidget(parent) {
    ...
    for(int i = 0; i < types.size(); ++i) {
        ...
        btn->setStyleSheet(QString(
            "QPushButton { background-color: %1; color: %2; border: 1px solid %3; font-family: 'Consolas'; font-weight: bold; font-size: 14px; }"
            "QPushButton:checked { background-color: %3; color: #000; border: 2px solid white; }"
        ).arg(theme.bg_dark.name()).arg(theme.secondary.name()).arg(theme.primary.name()));
