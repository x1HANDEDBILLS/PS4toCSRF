...
void Frame::set_params(float edge, float border, int alpha) {
    m_edge = edge; m_border = border; m_alpha = alpha;
    update();
}
...
void Frame::paintEvent(QPaintEvent *event) {
    ...
    QPainterPath path;
    path.moveTo(m_edge, 0); path.lineTo(w - m_edge, 0); path.lineTo(w, m_edge);
    path.lineTo(w, h - m_edge); path.lineTo(w - m_edge, h); path.lineTo(m_edge, h);
    path.lineTo(0, h - m_edge); path.lineTo(0, m_edge); path.closeSubpath();
