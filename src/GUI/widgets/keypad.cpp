...
void TacticalKeypad::handle_key() {
    ...
    else if(t == "BACK") display->setText(display->text().left(display->text().length()-1));
    else display->insert(t);
}

void TacticalKeypad::refresh_theme() {
    QString h = ThemeManager::instance().active().hex;
    display->setStyleSheet(QString("background:#000; color:white; border:1px solid %1; font-family:'Consolas'; font-size:24px; padding:5px;").arg(h));
    for(Button *b : keys) {
